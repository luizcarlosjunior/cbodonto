...
export default function Footer({ settings }: FooterProps) {
  const { whatsappNumber: waNumber, cnpj, legalName, customerServiceEmail, footerText, instagramUrl, facebookUrl } = settings

  return (
    <footer className="bg-[#0f0d0b] border-t border-gold/10 py-12">
      <div className="max-w-[1600px] mx-auto px-[8%]">
        <div className="flex flex-col md:flex-row items-start justify-between gap-12 mb-12">
          ...
          <div className="flex flex-col gap-2">
            <h4 className="text-gold text-[0.65rem] tracking-widest uppercase mb-2">Contato</h4>
            {customerServiceEmail && (
              <a href={`mailto:${customerServiceEmail}`} className="text-stone-400 text-sm hover:text-white transition-colors">
                {customerServiceEmail}
              </a>
            )}
            <a href={`https://wa.me/${waNumber}`} className="text-stone-400 text-sm hover:text-white transition-colors">
              WhatsApp: ({waNumber.slice(2, 4)}) {waNumber.slice(4, 9)}-{waNumber.slice(9)}
            </a>
          ...
          {/* Links */}
          <div className="flex flex-col gap-2">
            <h4 className="text-gold text-[0.65rem] tracking-widest uppercase mb-2">Navegação</h4>
            <div className="grid grid-cols-2 gap-x-8 gap-y-2">
              {[
                ['#servicos', 'Tratamentos'],
                ['#sobre', 'A Clínica'],
                ['#agendamento', 'Agendar'],
                ['/admin/login', 'Acesso Restrito'],
              ].map(([href, label]) => (
                <a
                  key={label}
                  href={href}
        ...
        <div className="pt-8 border-t border-gold/10 flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="text-center md:text-left">
            <p className="text-stone-600 text-[0.7rem] tracking-wide mb-1">
              © {new Date().getFullYear()} {legalName || 'Campozano & Barcellos Odontologia'}
            </p>
            {cnpj && (
              <p className="text-stone-700 text-[0.65rem] tracking-wide">
                CNPJ: {cnpj} · Colombo, PR
              </p>
            )}
          </div>
          
          <div className="flex gap-6">
            <a href="politica-de-privacidade" className="text-stone-600 text-[0.7rem] tracking-widest uppercase hover:text-gold transition-colors">Privacidade</a>
            <a href="termos-de-uso" className="text-stone-600 text-[0.7rem] tracking-widest uppercase hover:text-gold transition-colors">Termos</a>
          </div>
        </div>
      </div>
    </footer>
