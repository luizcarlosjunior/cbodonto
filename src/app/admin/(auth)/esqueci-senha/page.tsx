'use client'
// src/app/admin/(auth)/esqueci-senha/page.tsx
...
export default function EsqueciSenhaPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) { setError(data.error ?? 'Erro ao processar solicitação'); setLoading(false); return }
      setSent(true)
    } catch {
  ...
  return (
    <div className="min-h-screen bg-stone-950 flex items-center justify-center px-4">
      <div className="w-full max-w-sm">
        ...
        {sent ? (
          <div className="bg-stone-900 border border-stone-800 p-8 text-center space-y-4">
            ...
            <h2 className="font-serif text-lg text-white">E-mail enviado</h2>
            <p className="text-stone-400 text-sm leading-relaxed">
              Se o endereço <strong className="text-stone-300">{email}</strong> estiver cadastrado,
              você receberá um link para criar uma nova senha. Verifique também a pasta de spam.
            </p>
            <p className="text-stone-600 text-xs">O link expira em 1 hora.</p>
            <Link
              href="/admin/login"
              className="inline-flex items-center gap-2 text-stone-400 hover:text-white text-xs transition-colors mt-2"
            >
              <ArrowLeft size={12} />
              Voltar para o login
            </Link>
          ...
          <form onSubmit={handleSubmit} className="bg-stone-900 border border-stone-800 p-8 space-y-5">
            <div>
              <h2 className="font-serif text-lg text-white mb-1">Recuperar senha</h2>
              <p className="text-stone-500 text-xs leading-relaxed">
                Informe seu e-mail de administrador e enviaremos um link para criar uma nova senha.
              </p>
            </div>
            ...
            <div className="text-center">
              <Link
                href="/admin/login"
                className="inline-flex items-center gap-2 text-stone-500 hover:text-stone-300 text-xs transition-colors"
              >
                <ArrowLeft size={12} />
                Voltar para o login
              </Link>
