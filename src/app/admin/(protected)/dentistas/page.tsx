...
import { Plus, UserCheck, UserX, Calendar, Mail, Phone, Search, X } from 'lucide-react'
...
export default function DentistasPage() {
  ...
  const fetchDentists = useCallback(async (pg = page) => {
    setLoading(true)
  ...
  useEffect(() => { fetchDentists(1) }, [search]) // eslint-disable-line react-hooks/exhaustive-deps
  useEffect(() => { fetchDentists() }, [page])    // eslint-disable-line react-hooks/exhaustive-deps
    ...
    <div className="p-6 md:p-8 max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-stone-500 text-sm mt-0.5">{total} profissional(is) cadastrado(s)</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-white border border-stone-200 p-10 text-center text-stone-400">
          {search ? `Nenhum resultado para "${search}".` : 'Nenhum dentista cadastrado.'}
        </div>
      ) : (
        <>
          <div className="grid md:grid-cols-2 gap-4">
